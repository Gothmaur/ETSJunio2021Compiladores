    ...
    while(i < (int)subconjuntos.size()){
        for(int j = 0 ; j < (int)afn.automata_alfabeto.size() ; j++){
            ...
            if(kernel.size() > 0 && !existeElKernel()){ // Se busca si el kernel ya existe
                cout << "Mover (Q" << i << ", " << afn.automata_alfabeto[j] << ")" << endl;//Imprime movimientos a analizar
                ...
                if(!existeElSubconjunto()){ // Buscamos si existe el subconjunto
                    ...
                    subconjunto.clear(); //Limpia mi subconjunto
                    agregarTransicionAFD(i,subconjuntoActual,afn.automata_alfabeto[j]); //Agrega trancisión de q(i) al q(sbca) con el simbolo analizado
                    k--;
                ...
                if(kernel.size() > 0){  // Comprobamos que no sea un kernel sin elementos
                    cout << "Mover (Q" << i << ", " << afn.automata_alfabeto[j] << ")" << endl;//Mover Q(i) con simbolo del alfabeto
                    imprimirMoverConSimbolo();//Imprime el kernel al que se mueve
                    agregarTransicionAFD(i,kernelActual,afn.automata_alfabeto[j]);//Agregar Q(i) -> q(kernel) con simbolo
                    kernel.clear();//Limpia el kernel
                }
            }
        }
...
bool Subconjunto::existeElKernel(){
    ...
    vector<int>::iterator it;
    for(int i = 0 ; i < j ; i++){
        if(kernel.size() == kernels[i].size()){//Si el tamaño del kernel coincide con un kernel existente
            diferencia.reserve(kernel.size()*2); //Reserva espacio para 2 kernel
            it = set_difference(kernel.begin(),kernel.end(),kernels[i].begin(),kernels[i].end(),diferencia.begin());
            //Almacena los valores diferentes entre kernels para comparar en diferencia
            diferencia.resize(it-diferencia.begin());//redimenciona el vector Diferencia
            if(diferencia.size() == 0){//Si no se encontró ningún elemento distinto (Son kernels identicos)
                kernelActual = i+1; //Identifica el kernel para uso posterior
                return true; //regresa true
            }
            else{
                diferencia.clear();//Limpia el vector diferencia para comparar otro kernel
            }
        }
    }
    return false;
...
void Subconjunto::agregarKernel(){
    kernels.reserve(1);
    kernels.push_back(kernel);//Agrega el kernel a la pila
    imprimirMoverConSimbolo();//Imprime mi kernel (a donde se movió con el valor analizado)
    kernel.clear();//Limpia el kernel para evaluar uno nuevo
}
